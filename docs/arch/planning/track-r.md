...
**담당자 결정:** ref count·lease 상태·pool 내부 표현 및 ledger 테스트

**협의 조건:** Checkout/Checkin·root 인계·scope 종료·외부 자원 한도를 바꿀 때

**협의 대상·관련 경계:** R의 Queue/Executor/Workspace 담당과 I; 종료 H — DC-04 ([결정 범위](../14-decision-boundaries.md)). 기존 계약 준수 시 재협의 없이 진행한다.
...
**담당자 결정:** FIFO를 지키는 큐 표현·no_policy 구현·테스트

**협의 조건:** reader/writer 소유권·포화 결과·동시 접근·종료 규칙을 바꿀 때

**협의 대상·관련 경계:** I와 R의 Lifetime/Executor 담당; 설정/종료 H — DC-04, DC-05, DC-09 ([결정 범위](../14-decision-boundaries.md)). 기존 계약 준수 시 재협의 없이 진행한다.
...
**담당자 결정:** 이름 조회 자료구조·registry 내부 코드

**협의 조건:** 등록 완료·미등록/중복 대상·호출 대상 결과를 바꿀 때

**협의 대상·관련 경계:** R의 Executor/Workspace 담당; plugin 로딩 H, 진단 E — DC-05 ([결정 범위](../14-decision-boundaries.md)). 기존 계약 준수 시 재협의 없이 진행한다.
...
**담당자 결정:** 공개 계약 안의 예제 파싱·record 변환과 내부 클래스

**협의 조건:** Workspace 공개 API·lease 수명·record 의미를 바꿀 때

**협의 대상·관련 경계:** R의 Lifetime/Executor 담당과 P; 관리 plugin A — DC-04, DC-06 ([결정 범위](../14-decision-boundaries.md)). 기존 계약 준수 시 재협의 없이 진행한다.
...
**담당자 결정:** 기본 순차 loop·context 생성·정리 코드

**협의 조건:** 호출 동시성·Process 완료·실패 후 다음 대상·정지 규칙을 바꿀 때

**협의 대상·관련 경계:** R의 Queue/Lifetime/Workspace 담당; 저장 P, 종료 H, 오류 E — DC-04, DC-05, DC-09 ([결정 범위](../14-decision-boundaries.md)). 기존 계약 준수 시 재협의 없이 진행한다.
...
- [ ] `R-06.3` 테스트용 ledger로 Workspace 반납과 root 회수 순서 확인
...
**담당자 결정:** 대역·테스트 ledger·검증 실행 방법

**협의 조건:** 공통 참조 수명·순차 실행·record 기대 결과를 바꿀 때

**협의 대상·관련 경계:** 해당 R 모듈 담당과 P; 전체 인수 X — DC-04, DC-05, DC-06 ([결정 범위](../14-decision-boundaries.md)). 기존 계약 준수 시 재협의 없이 진행한다.
