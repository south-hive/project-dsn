...
**개발 계약 보완:** GAP-08: 선택 가능한 field 목록, 정의 보존, 사용자 구분, 빈 결과·페이지·잘못된 field 규칙을 명시한다. [점검 목록](../12-engineering-readiness.md)

**담당자 결정:** View schema·API·결과 예제 초안 작성

**협의 조건:** field 목록·결합 규칙·정의 보존·사용자 구분·원격 API를 정할 때

**협의 대상·관련 경계:** P; record 생성 의미 R/A, endpoint 배포 H — DC-07 ([결정 범위](../14-decision-boundaries.md)). 기존 계약 준수 시 재협의 없이 진행한다.
...
**담당자 결정:** 정해진 결과를 만드는 내부 조합·query 변환 구현

**협의 조건:** 조회 의미·field 타입·페이지·결과 표현의 공개 계약을 바꿀 때

**협의 대상·관련 경계:** P 및 View API 소비자 — DC-07, DC-06 ([결정 범위](../14-decision-boundaries.md)). 기존 계약 준수 시 재협의 없이 진행한다.
...
**담당자 결정:** 기존 endpoint·저장 계약의 연결 코드와 테스트

**협의 조건:** 원격 노출·사용자 범위·저장소 query 의미를 바꿀 때

**협의 대상·관련 경계:** P·H 및 View API 소비자; 인수 X — DC-07, DC-10 ([결정 범위](../14-decision-boundaries.md)). 기존 계약 준수 시 재협의 없이 진행한다.
