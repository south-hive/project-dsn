...
실행 근거는 **2026-09-08** Termux Android arm64, .NET SDK 10.0.111, .NET/ASP.NET Core 10.0.11, `linux-bionic-arm64` Mono 환경이다. 2026-09-09 문서 재작성 때 애플리케이션 테스트를 다시 실행한 결과가 아니다.

- Release build: 경고 0, 오류 0. [기능·통합 테스트](../tests/Dsn.Tests/Program.cs) 13개 그룹 통과.
...
| QA2 / 메모리 | 동시 중복 Checkin, 반납 후 접근, 실패 정리, 저장 전 반납 | 시험 사례에서 단일 반환과 최종 참조 0. 장기 부하/GC·pool 동작 일반화 불가 |
...
| QA7 / 변경 | 실제 예제 DLL 로딩·처리·필수 파일 누락 실패 | 예제 plugin 경로 확인. 부적합 API 버전/복잡한 의존 DLL 조합의 검증 확대 필요 |
...
| 신뢰 경계와 Host 집중 | 단순 plugin/HTTP 조립 | 신뢰하지 않는 plugin 실행 위험, HTTP 변경이 lifecycle 파일과 겹침 | 필요 시 격리 수준과 HTTP 모듈 분리 검토 |
