...
public sealed record DecodedMessage(Envelope Envelope, byte[] Payload);
public interface IMessageDecoder { DecodedMessage Decode(JsonElement value); }
public sealed class Version1Decoder(int maxPayloadBytes = 16384) : IMessageDecoder
{
    ...
    public DecodedMessage Decode(JsonElement p)
    {
        try
        {
            ...
            if (targets.Length is < 1 or > 32 || targets.Any(x => !Names.Valid(x))) throw new FormatException();
...
{
    public static bool Valid(string? value) => value is not null && Regex.IsMatch(value, "^[a-z][a-z0-9-]{0,63}$");
...
public sealed class Protocol
{
    ...
    public Protocol(int maxPayloadBytes = 16384) : this(new Dictionary<int, IMessageDecoder> { [1] = new Version1Decoder(maxPayloadBytes) }) { }
    public Protocol(IReadOnlyDictionary<int, IMessageDecoder> decoders) => this.decoders = new Dictionary<int, IMessageDecoder>(decoders);
    public DecodedMessage Decode(ReadOnlyMemory<byte> frame)
    {
        JsonDocument doc;
