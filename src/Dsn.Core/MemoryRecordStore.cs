...
public sealed class MemoryRecordStore(int capacity = 1000, long maxBytes = 4 * 1024 * 1024) : IRecordStore, IRecordQuery, IRecordExporter
{
    ...
    public ValueTask AppendAsync(RecordInput input, CancellationToken cancellationToken = default)
    {
        ...
        lock (gate)
        {
            var row = JournalStore.Freeze(new(rows.Count + 1L, input.Workspace, input.Fields));
            var length = JsonSerializer.SerializeToUtf8Bytes(row, Json.Options).Length + 1;
