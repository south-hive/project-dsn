...
public sealed class ViewService(IRecordQuery records)
{
    ...
    {
        if (view.Workspaces is null || view.Fields is null || view.Workspaces.Length is < 1 or > 32 || view.Workspaces.Any(w => !Names.Valid(w)) ||
            view.Fields.Length is < 1 or > 32 || view.Fields.Any(f => !Names.Field(f))) throw new ArgumentException("Invalid View definition");
...
public sealed class ViewDefinitions
{
    ...
    {
        this.path = path;
        data = File.Exists(path) ? JsonSerializer.Deserialize<Dictionary<string, Dictionary<string, ViewDefinition>>>(File.ReadAllText(path), Json.Options) ?? throw new InvalidDataException("Invalid views") : [];
    }
    ...
    {
        if (!Names.Valid(name)) throw new ArgumentException("Invalid View name");
        lock (gate)
        {
